...
class GameRepository {
    ...
    suspend fun getAllGames(uid: String) : List<GameList> {
        ...
        val data = db.collection("users")
            ...
            games.map { game ->
                game.gameImage = getGameImage(uid,game.gameName?.toLowerCase()?.replace(" ","_") ?: "")
            }
    ...
    suspend fun saveGame(gameList: GameList, uid: String) {
        try {
            ...
            // salva imagem no storege do Firebase
            var metadata = storageMetadata {
                contentType = "image/jpg"
            }

            val fileRef = storageRef.child("${uid}/${gameList.gameName?.toLowerCase()?.replace(" ","_")}.jpg" ?: "")
